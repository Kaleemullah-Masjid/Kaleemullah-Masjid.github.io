...
async function fetchJuzData(random_juz) {
    ...
    try {
        ...
        if (response.ok) {
            // Extract English translation data for the given Juz number
            const en_juz_data = data[random_juz]
            return (en_juz_data)
        }
        else {
...
async function clean_words(string_chars) {
    ...
    const filteredWords = words.filter(function (word) {
        ...
        return word.length > 0 && !stopWords.includes(word.toLowerCase());
    });
    // Display the word count
    var wordCount = filteredWords.length;
...
async function addText(word_freq_dict) {
    ...
    for (var key in word_freq) {
        ...
        if (value < 10) {
            ...
            var fontSize = value - 20;
            ctx.font = fontSize + "px Arial";
        }
        // Check if the next line exceeds the canvas height
        if (y + fontSize > canvas.height) {
            // Reset y to a new starting position
            y = 30;
            // Move to a new column
            x += 100; 
        }
        // Check if the word exceeds the canvas width
        if (x + ctx.measureText(key).width > canvas.width) {
            x = 10; // Move to a new row
            y += lineHeight; // Move to the next line
        }

        ctx.fillText(key, x, y);
        // Update x position based on the width of the drawn word
